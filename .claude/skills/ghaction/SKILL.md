...
```bash
swift run ghaction auth status
```
...
swift run ghaction auth token --token ghp_xxxxxxxxxxxx

# Option 3: OAuth Device Flow (interactive)
swift run ghaction auth login --client-id <oauth-app-client-id>
```
...
```bash
swift run ghaction trigger -o <owner> -r <repo> -w <workflow.yml> [options]
```
...
```bash
swift run ghaction runs list -o <owner> -r <repo> [options]
```
...
```bash
swift run ghaction runs get -o <owner> -r <repo> <run-id>
```

### Watch Run

```bash
swift run ghaction runs watch -o <owner> -r <repo> <run-id> [options]
```
...
### Trigger CI and wait
```bash
swift run ghaction trigger -o igorcferreira -r GHActionTrigger -w ci.yml --wait
```

### Deploy with inputs
```bash
swift run ghaction trigger -o owner -r repo -w deploy.yml \
...
### Check recent runs
```bash
swift run ghaction runs list -o owner -r repo -w ci.yml --limit 5
```
